...
class TimePartitionedPretrainedSTFTBridgeModel(LightningModule):
    ...
    def ddpm_sample(self, x_1, t_steps=None, mask=None, mask_pred_x0=True,
        ...
        # ⚡ Bolt: Vectorized temporal embeddings outside the loop
        t_embs_precalc = self.t_to_emb(t_steps[0, :n_steps]).unsqueeze(1).repeat(1, x_1.shape[0], 1)

        for t_idx in range(n_steps):
            # print(t_idx)
            t_emb = t_embs_precalc[t_idx]
            t = t_steps[:, t_idx]
