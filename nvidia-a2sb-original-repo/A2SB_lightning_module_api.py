...
class TimePartitionedPretrainedSTFTBridgeModel(LightningModule):
    ...
    def ddpm_sample(self, x_1, t_steps=None, mask=None, mask_pred_x0=True,
        ...
        # ⚡ Bolt: Precalculate all temporal embeddings in a single batched operation
        # This replaces O(N) network calls with a single O(1) vectorized operation
        precalc_t_embs = self.t_to_emb(t_steps[0, :n_steps])

        for t_idx in range(n_steps):
            # print(t_idx)
            t_emb = precalc_t_embs[t_idx].unsqueeze(0).repeat(x_1.shape[0], 1)
            t = t_steps[:, t_idx]
