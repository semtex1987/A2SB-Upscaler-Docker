...
class MixAudioDataset(torch.utils.data.Dataset):
    ...
    def load_wav_to_torch(self, audiopath, start_time=None, end_time=None):
        ...
        # ⚡ Bolt: Calculate valid bounds to avoid out-of-bounds loading
        valid_start = max(0.0, min(start_time, total_duration - duration_sec))

        # ⚡ Bolt: Request slightly larger duration to guarantee sufficient frames remain after resampling
        fetch_duration = duration_sec + 0.05

        # ⚡ Bolt: Lazy fetch! Dramatically reduces I/O and resampling overhead for large files.
        ...
        # ⚡ Bolt: Explicitly slice/pad the output array to the exact expected length
