...
class MixAudioDataset(torch.utils.data.Dataset):
    ...
    def load_wav_to_torch(self, audiopath, start_time=None, end_time=None):
        # ⚡ Bolt: Use lazy loading to avoid reading entire audio file. Calculate total duration first.
        total_duration = librosa.get_duration(path=audiopath)
        duration_sec = self.segment_length / self.sampling_rate
        # Ensure start_time does not exceed valid boundaries
        valid_start = max(0.0, float(min(start_time, total_duration - duration_sec)))

        # Request slightly larger duration to guarantee sufficient frames after resampling, then slice to exact length
