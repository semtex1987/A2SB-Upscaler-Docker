...
class MixAudioDataset(torch.utils.data.Dataset):
    ...
    def load_wav_to_torch(self, audiopath, start_time=None, end_time=None):
        # ⚡ Bolt: Use lazy loading to avoid reading full audio files into memory.
        # Compute valid segment boundaries using librosa.get_duration to prevent out-of-bounds errors.
        ...
        safe_start = max(0.0, min(start_time, file_duration - duration_sec))

        audio, sr = librosa.load(audiopath, sr=None, offset=safe_start, duration=duration_sec)
