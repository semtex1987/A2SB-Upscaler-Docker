...
class MixAudioDataset(torch.utils.data.Dataset):
    ...
    def load_wav_to_torch(self, audiopath, start_time=None, end_time=None):
        # ⚡ Bolt: Lazily load only the required segment before resampling to avoid massive memory/compute overhead
        ...
        # Preserve original clamping: shift start time backward if it exceeds total_duration
        valid_start = max(0.0, min(start_time, total_duration - duration_sec))

        # Load slightly more than required to account for resampling artifacts/rounding
        audio, sr = librosa.load(audiopath, sr=None, offset=valid_start, duration=duration_sec + 0.05)
