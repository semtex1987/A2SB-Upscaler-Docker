...
class MixAudioDataset(torch.utils.data.Dataset):
    ...
    def load_wav_to_torch(self, audiopath, start_time=None, end_time=None):
        ...
        valid_start = max(0.0, min(start_time, file_duration - target_duration))

        buffer_sec = 0.05
        read_start = max(0.0, valid_start - buffer_sec)
        read_duration = target_duration + 2 * buffer_sec

        audio, sr = librosa.load(audiopath, sr=None, offset=read_start, duration=read_duration)
        if len(audio.shape) != 1:
            audio = librosa.to_mono(audio.T)  # (L, 2) -> (2, L) -> mono-channel
        if sr != self.sampling_rate:
            audio = librosa.resample(audio, orig_sr=sr, target_sr=self.sampling_rate)

        offset_frames = floor(valid_start * self.sampling_rate) - floor(read_start * self.sampling_rate)
        offset_frames = max(0, offset_frames)

        audio = audio[offset_frames:offset_frames+self.segment_length]
