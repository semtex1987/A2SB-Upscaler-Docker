...
class MixAudioDataset(torch.utils.data.Dataset):
    ...
    def load_wav_to_torch(self, audiopath, start_time=None, end_time=None):
        ...
        duration_sec = (end_time - start_time) if end_time else (self.segment_length / self.sampling_rate)

        # Calculate total duration to prevent out-of-bounds loading
        total_duration = librosa.get_duration(path=audiopath)
        valid_start = max(0.0, min(start_time, total_duration - duration_sec))

        # ⚡ Bolt: Lazy load only the requested segment (with a tiny buffer for resampling math)
        ...
        # Slice to the exact expected length
