...
from diffusion import (
    ...
    get_multidiffusion_vf,
    multidiffusion_pad_inputs,
    multidiffusion_unpad_outputs,
)
...
class TimePartitionedPretrainedSTFTBridgeModel(LightningModule):
    def __init__(
        ...
        # SimpleUnet
        self.t_to_emb = SinusoidalTemporalEmbedding(
            n_bands=int(n_timestep_channels // 2), min_freq=0.5
        )
        if self.use_ot_ode:
    ...
    def load_t_bounded_checkpoints(self, pretrained_checkpoints, t_cutoffs):
        ...
        self.t_bounded_pretrained_models = nn.ModuleList(
            [_load_model(ckpt) for ckpt in pretrained_checkpoints]
        )
    ...
    def vocode_stft(self, spec_out):
        ...
        return [
            apply_audio_transforms(spec_out[b], self.inv_transforms)[0]
    ...
    def ddpm_sample(
        ...
        # ⚡ Bolt: Vectorize temporal embedding calculation across all timesteps outside the loop
        ...
        for t_idx in tqdm(range(n_steps)):
            ...
            x_t_prev = self.ddpm.p_posterior(
                t_prev, t, x_t, pred_x0, ot_ode=self.use_ot_ode
            )
            x_t = x_t_prev
            if mask is not None:
                xt_true = x_1
                ...
                x_t = (1.0 - mask) * xt_true + mask * x_t
        all_pred_x0s = [
            multidiffusion_unpad_outputs(pred, original_width) for pred in all_pred_x0s
        ]
        return all_pred_x0s

    @torch.no_grad()
    def fast_inpaint_ddpm_sample(
        self,
        x_1,
        t_steps=None,
        mask=None,
        ...
        x_1 = x_1.clone()
        x_1 = multidiffusion_pad_inputs(x_1, win_length, hop_length)
        mask = multidiffusion_pad_inputs(
            mask, win_length, hop_length, padding_constant=0
        )
    ...
    def predict_step(self, batch, batch_idx):
        ...
        t_steps = (
            torch.linspace(1, 0.05, int(self.predict_n_steps))
            .unsqueeze(0)
            .to(x_0_corrupted.device)
        )
        if not self.fast_inpaint_mode:
            x_0s = self.ddpm_sample(
                x_0_corrupted,
                t_steps=t_steps,
                mask=mask,
                mask_pred_x0=True,
            ...
            x_0s = self.fast_inpaint_ddpm_sample(
                x_0_corrupted,
                t_steps=t_steps,
                mask=mask,
                mask_pred_x0=True,
        ...
        write_wav(
            os.path.join(current_out_dir, "recon.wav"),
            batch["output_sr"],
            reconstructed_audio,
        )
        write_wav(
            os.path.join(current_out_dir, "dirty.wav"), batch["output_sr"], input_audio
        )
...
class STFTBridgeModel(LightningModule):
    def __init__(
        ...
        self.inv_transforms = (
            inv_transforms  # inverse transforms to map spectrograms back to waveform
        )
    ...
    def ddpm_sample(self, x_1, t_steps=None, mask=None, mask_pred_x0=True):
        ...
        # ⚡ Bolt: Vectorize temporal embedding calculation
        t_embs = self.t_to_emb(t_steps[0, :n_steps])
        ...
        for t_idx in range(n_steps):
            ...
            if mask is not None:
                ...
                if not self.use_ot_ode:
                    ...
                    coef1, coef2, var = compute_gaussian_product_coef(
                        sigma_fwd, sigma_rev
                    )
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = (
            ((vf_output - target.detach()) ** 2) * loss_mask
        ).sum() / loss_mask.sum()
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        t_steps = (
            torch.linspace(1, 1.0 / n_steps, int(n_steps))
            .unsqueeze(0)
            .to(x_0_corrupted.device)
        )

        x_0s = self.ddpm_sample(
            x_0_corrupted, t_steps=t_steps, mask=loss_mask, mask_pred_x0=True
        )
        ...
        for idx in range(n_samples):
            result = metrics.evaluation(
                inpainted_audio[idx].cpu().numpy(),
                batch["x_0_wav"][idx].cpu().numpy(),
                None,
            )
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        try:
            ...
            for scale in scales:
                ...
                t_emb = self.t_to_emb(t)
                x_t = self.ddpm.q_sample(
                    t, x_0_clean, x_0_corrupted, ot_ode=self.use_ot_ode
                )
                std_fwd_t = self.ddpm.get_std_fwd(t)
                vf_output = self.vf_model(x_t, t_emb)
                # apply mask
    ...
    def on_validation_batch_end(
        ...
        if pl_module.global_rank == 0 and batch_idx == 0:
            for i in range(num_to_plot):
                ...
                x_0_corrupted = (
                    sample["x_0_corrupted"].unsqueeze(0).to(pl_module.device)
                )
                x_0_corrupted_mag = pl_module.inv_transforms[0](
                    self.get_mag(x_0_corrupted)
                )
                x_0_clean = (
                    sample["x_0_clean"].unsqueeze(0).to(pl_module.device)
                )  # otherwise some asynch transforms don't get run
                ...
                pl_module.logger.experiment.add_audio(
                    sample_id + "Original Audio",
                    gt_reconstruction[0].data.numpy(),
                    pl_module.global_step,
                    pl_module.sampling_rate,
                )

                pl_module.logger.experiment.add_image(
                    sample_id + "Original Magnitude",
                    plot_spec_to_numpy(x_0_clean_mag[0].data.cpu().numpy()),
                    pl_module.global_step,
                    dataformats="HWC",
                )
                pl_module.logger.experiment.add_image(
                    sample_id + "Masked Magnitude",
                ...
                n_steps = 25
                t_steps = (
                    torch.linspace(1, 0.05, n_steps)
                    .unsqueeze(0)
                    .to(x_0_corrupted.device)
                )
                x_0s = pl_module.ddpm_sample(
                    x_0_corrupted.unsqueeze(0), mask=mask, t_steps=t_steps
                )
                sampled_spec = x_0s[-1]
                sampled_spec_mag = self.get_mag(
                    pl_module.inv_transforms[0](sampled_spec[0])
                )
                pl_module.logger.experiment.add_image(
                    sample_id + "Inpainted Magnitude",
                ...
                pl_module.logger.experiment.add_audio(
                    sample_id + "Inpainted Audio",
                    inpainted_audio[0].data.numpy(),
                    pl_module.global_step,
                    pl_module.sampling_rate,
                )
