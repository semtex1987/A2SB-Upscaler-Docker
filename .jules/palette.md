...
## 2026-08-03 - Expanded Hit Areas for List View Checkboxes
**Learning:** In list views where users select items via checkboxes, the default 16x16px click target violates Fitts's Law. Always wrap the adjacent row content in a `<label htmlFor={id}>` with `cursor-pointer` to expand the hit area to the full row contents without requiring custom JavaScript.
**Action:** Use labels to expand checkbox hit areas.
