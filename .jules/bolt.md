...
## 2024-07-07 - Optimize macroscopic STFT heuristic aggregations
**Learning:** Functions that aggregate STFT data macroscopically (e.g., global RMS or mean magnitude over time) do not require the standard 75% frame overlap (hop_length = n_fft // 4). Using this default overlap computes approximately 4x as many frames as a 0% overlap approach.
**Action:** For macroscopic STFT heuristics, reduce overlap to 50% (`hop_length=n_fft//2`) to halve computation time while avoiding blind spots or severe spectral leakage (which can occur with 0% overlap depending on the windowing function). Avoid reducing overlap for STFTs used for visualization, as it degrades time resolution.
