...
## 2024-05-23 - Fast heuristic spectral metrics
**Learning:** `librosa.feature.spectral_flatness` and `librosa.feature.spectral_rolloff` are severe computational bottlenecks with default arguments (`hop_length=512`, `n_fft=2048`) due to the 75% STFT overlap. When these are used for macroscopic heuristic checks (like checking the mean value to detect noise), this overlap is redundant.
**Action:** Explicitly increase both `hop_length` and `n_fft` (e.g., to 2048) when calculating these features for macroscopic heuristic checks to remove the overlap, making calculations roughly 4x faster.
