...
## 2025-05-18 - Avoid librosa.feature.spectral_flatness default parameters
**Learning:** `librosa.feature.spectral_flatness` is a severe computational bottleneck with default arguments (`hop_length=512`, `n_fft=2048`).
**Action:** To drastically speed up computation for macro-level metric approximations (like overall noise checks), explicitly increase `hop_length` and `n_fft` (e.g., to 2048) when calling `librosa.feature.spectral_flatness`.
