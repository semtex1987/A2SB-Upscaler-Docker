...
## 2024-05-18 - Optimize librosa STFT hop_length for macroscopic heuristics
**Learning:** When calculating macroscopic heuristic functions that aggregate STFT data (like percentiles or means over time), reducing the overlap from 25% (e.g. `hop_length=1024` when `n_fft=4096`) to 50% (`hop_length=n_fft//2`) speeds up computation by ~2-4x while maintaining sufficient signal integrity. (Going to 0% overlap may fail strict parity tests).
**Action:** Use `hop_length=n_fft//2` instead of hardcoded lengths for coarse STFT aggregations to maximize speed without breaking logic.
