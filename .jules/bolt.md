...
**Learning:** `librosa.load` reads the entire file into memory by default before returning a numpy array. When loading short segments from very large audio files, this introduces an enormous and redundant IO and resampling bottleneck.
**Action:** When extracting short segments from large files, always compute valid segment boundaries using `librosa.get_duration` first to prevent out-of-bounds reading. Then, utilize the `offset` and `duration` arguments of `librosa.load` to lazily fetch only the necessary frames, dramatically reducing execution time.
