...
## 2026-07-09 - Lazy load audio segments before resampling
**Learning:** When extracting and resampling short audio segments from large files, using `librosa.load` with its defaults reads the entire file into memory before any cropping or resampling. This results in massive, redundant overhead and memory usage.
**Action:** Compute valid segment boundaries using `librosa.get_duration(path=...)`, then lazily fetch only the required frames by passing `offset` and `duration` arguments to `librosa.load` before applying `librosa.resample`. Request a slightly larger duration to guarantee sufficient frames, then slice the output exactly.
