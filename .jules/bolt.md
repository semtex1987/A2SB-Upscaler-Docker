...
## 2025-10-24 - Vectorize piece-wise max-pooling for spectrogram generation
**Learning:** When performing piece-wise aggregations (like max-pooling) across a dimension in large numpy arrays using predefined edge bins, using list comprehensions with `np.stack` creates a massive bottleneck.
**Action:** Use native C-level vectorization via `np.maximum.reduceat(matrix, edges[:-1], axis=...)` to achieve significant speedups for max-pooling segments.
