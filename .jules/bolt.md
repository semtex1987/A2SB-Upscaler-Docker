...
## 2025-02-28 - Optimize librosa feature extraction defaults
**Learning:** `librosa.feature.spectral_flatness` and `librosa.feature.spectral_rolloff` are severe computational bottlenecks with default arguments (`hop_length=512`, `n_fft=2048`). The dense overlapping windows cause these heuristic checks to take longer than actual inference.
**Action:** Explicitly increase both `hop_length` and `n_fft` (e.g., to 2048) when calculating these macroscopic features. Removing the 75% STFT overlap makes the calculations roughly 4-8x faster without meaningfully degrading accuracy.
