...
## 2026-08-02 - Optimize macroscopic STFT aggregations
**Learning:** While reducing `librosa.stft` overlap to 0% (`hop_length=n_fft`) yields maximum speed for macroscopic heuristic functions, it alters the calculated curves too aggressively, failing strict test assertions and potentially compromising signal bounds. To safely optimize STFT aggregations without regressions, fallback to a 50% overlap (`hop_length=n_fft//2`) for a safe ~2x speedup.
**Action:** Always test overlap optimizations against parity tests. Apply `hop_length=n_fft//2` by default for macroscopic heuristics, but never apply it to visual features like spectrograms where time resolution is critical.
