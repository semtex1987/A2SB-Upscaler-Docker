...
## 2025-05-21 - Optimize librosa spectral feature extraction
**Learning:** `librosa.feature.spectral_flatness` and `librosa.feature.spectral_rolloff` are severe computational bottlenecks with default arguments (`hop_length=512`, `n_fft=2048`). The default 75% STFT overlap creates significant unnecessary overhead for simple heuristic checks.
**Action:** Explicitly increase both `hop_length` and `n_fft` (e.g., to 2048) when calculating these features for macroscopic heuristic checks to remove the overlap, making calculations roughly 4x faster without significantly altering the results.
