...
## 2026-07-18 - Optimize STFT overlap for heuristics
**Learning:** Replacing an STFT bin-average RMS calculation with a time-domain IIR filter changes the mathematical output entirely. To safely optimize macroscopic heuristic functions that aggregate STFT data (like high-band RMS or HF edges), do not reduce overlap to 0% as it causes blind spots. Instead, use a 50% overlap (`hop_length=n_fft//2`) for a safe 2x speedup without sacrificing signal integrity.
**Action:** Set `hop_length=n_fft//2` to eliminate excess overlap when computing STFTs used for coarse heuristic checks that aggregate over time (e.g. mean mag).
