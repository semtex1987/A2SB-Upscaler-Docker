...
## 2025-02-28 - Avoid 75% overlap in librosa feature heuristics
**Learning:**  and  are severe computational bottlenecks when used with default arguments (, ). For broad heuristic checks that average the result, calculating 75% overlapping frames is a massive waste of CPU cycles.
**Action:** Explicitly increase both  and  (e.g., to 2048) when calculating macroscopic features in  to remove STFT overlap, achieving up to 4x speedup without significantly changing the aggregated outcome.

## 2024-06-04 - Avoid 75% overlap in librosa feature heuristics
**Learning:** `librosa.feature.spectral_flatness` and `librosa.feature.spectral_rolloff` are severe computational bottlenecks when used with default arguments (`hop_length=512`, `n_fft=2048`). For broad heuristic checks that average the result, calculating 75% overlapping frames is a massive waste of CPU cycles.
**Action:** Explicitly increase both `hop_length` and `n_fft` (e.g., to 2048) when calculating macroscopic features in `librosa` to remove STFT overlap, achieving significant speedup without significantly changing the aggregated outcome.
