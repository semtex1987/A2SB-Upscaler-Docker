...
## 2026-08-08 - Vectorize piece-wise aggregations
**Learning:** When performing piece-wise aggregations (like max-pooling) across a dimension in large numpy arrays using predefined edge bins, using list comprehensions with `np.stack` creates a massive bottleneck. Native C-level vectorization is required for performance.
**Action:** Use native C-level vectorization via `np.maximum.reduceat(matrix, edges[:-1], axis=...)` instead of loops or comprehensions to achieve significant speedups.
