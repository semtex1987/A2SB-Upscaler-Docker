...
**Learning:** `librosa.feature.spectral_flatness` and `librosa.feature.spectral_rolloff` are severe computational bottlenecks with default arguments (`hop_length=512`, `n_fft=2048`). This leads to excessive temporal resolution which is unnecessary when computing macroscopic heuristics like taking the mean of the entire file.
**Action:** Explicitly increase both `hop_length` and `n_fft` (e.g., to 2048) when calculating these features for macroscopic heuristic checks. This removes the 75% STFT overlap and makes calculations roughly 4-5x faster.
