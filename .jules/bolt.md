...
## 2026-07-08 - Optimize STFT overlap for heuristic aggregations
**Learning:** Replacing an STFT bin-average RMS calculation with a time-domain IIR filter changes the mathematical output entirely. To safely optimize macroscopic heuristic functions that aggregate STFT data (like high-band RMS), do not reduce overlap to 0% (`hop_length=n_fft`) as it causes blind spots with a Hann window or severe spectral leakage with a boxcar window.
**Action:** Instead, use a 50% overlap (`hop_length=n_fft//2`) for a safe 2x speedup without sacrificing signal integrity. Do not apply this optimization to visual features like spectrogram plots, as it degrades visual time resolution.
