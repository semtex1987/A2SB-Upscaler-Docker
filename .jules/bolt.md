...
## 2024-05-18 - Speed up spectral flatness computation
...
**Action:** To drastically speed up computation for macro-level metric approximations (like overall noise checks), explicitly increase `hop_length` and `n_fft` (e.g., to 2048) when exact fine-grained temporal resolution is not required.
