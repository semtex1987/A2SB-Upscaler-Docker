...
## 2026-08-03 - Optimize STFT frame overlap for macroscopic heuristics
**Learning:** When computing macroscopic heuristics that aggregate over time (e.g., percentiles or means), replacing an STFT bin-average RMS calculation with a time-domain IIR filter changes the mathematical output entirely. To safely optimize macroscopic heuristic functions that aggregate STFT data, do not reduce overlap to 0% (`hop_length=n_fft`) as it causes blind spots with a Hann window or severe spectral leakage with a boxcar window and can fail strict test assertions. Instead, use a 50% overlap (`hop_length=n_fft//2`) for a safe 2x speedup without sacrificing signal integrity.
**Action:** Set `hop_length=n_fft//2` to eliminate redundant STFT frames when extracting spectral features for macroscopic heuristic checks.
