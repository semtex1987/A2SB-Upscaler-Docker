...
## 2026-06-13 - Optimize librosa spectral feature extraction
**Learning:** `librosa.feature.spectral_rolloff` and `librosa.feature.spectral_flatness` default to `hop_length=n_fft//4` which produces 75% STFT frame overlap. When aggregating over time, this overlap computes more frames than needed without added benefit.
**Action:** Set `hop_length=n_fft` to avoid the 75% overlap overhead when using these features for macroscopic checks like the 95th percentile or mean.
