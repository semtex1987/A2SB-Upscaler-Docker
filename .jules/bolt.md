...
## 2024-05-18 - Optimize librosa.stft hop_length for heuristic analysis
**Learning:** `librosa.stft` defaults to overlapping frames (e.g. 75% overlap when `n_fft=2048` and `hop_length=512`). For macroscopic heuristic checks like calculating the mean RMS of a frequency band, this overlap computes far more frames than needed, causing a massive performance bottleneck.
**Action:** Set `hop_length=n_fft` (e.g., `hop_length=2048`) to eliminate the overlap when using `librosa.stft` for macroscopic checks. This dramatically speeds up calculation without affecting the macro-scale result.
