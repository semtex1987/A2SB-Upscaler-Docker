...
## 2026-08-03 - Safe STFT Overlap Reduction
**Learning:** When computing macroscopic heuristics that aggregate over time (e.g., percentiles or means), setting `hop_length=n_fft` (0% overlap) provides a ~4x speedup but can slightly alter the calculated curve and fail strict test assertions.
**Action:** If parity tests fail with 0% overlap, fallback to a safer 50% overlap (`hop_length=n_fft//2`) for a ~2x speedup without sacrificing signal integrity.
