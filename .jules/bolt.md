...
## 2025-03-05 - Optimize spectral flatness calculation
**Learning:** `librosa.feature.spectral_flatness` with default arguments is a severe computational bottleneck. To drastically speed up computation for macro-level metric approximations (like overall noise checks), explicitly increase `hop_length` and `n_fft` (e.g., to 2048).
**Action:** Pass explicit, larger `hop_length` and `n_fft` values when approximating spectral metrics for performance.
