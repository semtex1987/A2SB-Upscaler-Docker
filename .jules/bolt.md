...
## 2025-02-28 - Use 50% overlap for STFT RMS aggregations
**Learning:** Replacing an STFT bin-average RMS calculation with a time-domain IIR filter changes the mathematical output entirely. To safely optimize macroscopic heuristic functions that aggregate STFT data (like high-band RMS), reducing overlap to 0% (hop_length=n_fft) causes blind spots with a Hann window or severe spectral leakage with a boxcar window.
**Action:** Use a 50% overlap (hop_length=n_fft//2) for a safe 2x speedup without sacrificing signal integrity in these heuristics.
