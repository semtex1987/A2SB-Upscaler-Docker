...
## 2024-07-27 - STFT 50% overlap for macroscopic aggregations
**Learning:** Replacing an STFT bin-average calculation with reduced overlap (0% / hop_length=n_fft) provides a 4x speedup but can severely alter the calculated curve and fail strict parity tests.
**Action:** When computing macroscopic heuristics that aggregate over time (e.g. percentiles or RMS) where 0% overlap fails tests, use a 50% overlap (hop_length=n_fft//2) for a safe 2x speedup without sacrificing test parity.
