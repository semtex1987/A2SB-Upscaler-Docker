...
## 2024-07-01 - Lazy Loading Audio Segments with librosa
**Learning:** Loading and resampling an entire audio file into memory just to crop a short segment is a massive performance bottleneck, especially for long files (e.g. 10+ minutes).
**Action:** When extracting short audio segments from large files, perform a lazy load using `librosa.load` with `offset` and `duration` arguments *before* applying `librosa.resample`. Calculate valid segment boundaries using `librosa.get_duration(path=...)` to prevent out-of-bounds errors. Request a slightly larger duration (e.g., `duration_sec + 0.05`) to guarantee sufficient frames remain after resampling, then slice the output array to exactly the required length to avoid downstream shape mismatch errors.
