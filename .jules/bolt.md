...
## 2026-07-28 - Optimize STFT frame overlap for aggregations
**Learning:** Replacing an STFT bin-average RMS calculation with a time-domain IIR filter changes the mathematical output. To safely optimize macroscopic heuristic functions that aggregate STFT data (like high-band RMS or percentiles), do not reduce overlap to 0% (`hop_length=n_fft`) as it causes blind spots with a Hann window. Instead, use a 50% overlap (`hop_length=n_fft//2`) for a safe 2x speedup without sacrificing signal integrity.
**Action:** Use 50% overlap for aggregations instead of the default 75% overlap.
