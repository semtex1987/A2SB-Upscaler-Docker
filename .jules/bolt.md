...
## 2023-10-27 - Extract vocode_stft to audio_utils
**Learning:** `vocode_stft` was a duplicated utility method on `A2SB_lightning_module` and `A2SB_lightning_module_api`.
**Action:** Extracted it into `audio_utils.py` and modified callers to pass `self.inv_transforms` directly, improving modularity and reducing code duplication without altering logic.
