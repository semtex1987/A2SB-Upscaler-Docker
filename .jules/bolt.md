...
## 2024-07-06 - Lazy loading before resampling in librosa
**Learning:** Loading and resampling entire large audio files just to crop a 1-second segment wastes enormous CPU time, memory, and introduces a severe performance bottleneck during dataset preparation.
**Action:** Use librosa.get_duration to compute boundaries safely, then use librosa.load with offset and duration arguments to lazily load only the required frames BEFORE resampling.
