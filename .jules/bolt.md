...
**Learning:** When extracting short audio segments from large files, `librosa.load` without `offset` and `duration` reads the entire file into memory, causing massive IO and CPU bottlenecks (especially if resampling is involved).
**Action:** Always compute valid segment boundaries using `librosa.get_duration(path=...)` and pass `offset` and `duration` arguments to `librosa.load` (adding a small buffer to `duration` to account for resampling drift, then slicing exactly to the target length) to lazily fetch only the required frames.
