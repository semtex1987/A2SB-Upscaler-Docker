...
## 2025-08-01 - 50% overlap for macroscopic STFT heuristics
**Learning:** When computing macroscopic heuristics that aggregate over time (e.g., percentiles or means), replacing the default 75% overlap (hop_length=n_fft//4) with a 50% overlap (hop_length=n_fft//2) provides a ~2x speedup without failing parity tests, as 0% overlap (hop_length=n_fft) alters the results too much.
**Action:** Safely use hop_length=n_fft//2 for heuristic STFT computations to reduce frames and speed up calculations.
