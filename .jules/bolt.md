...
**Action:** To drastically speed up computation for macro-level metric approximations (like overall noise checks), explicitly increase `hop_length` and `n_fft` (e.g., to 2048) to reduce redundant STFT operations without meaningfully altering the macro result.
