...
## 2026-07-28 - Optimize STFT overlaps for macroscopic heuristics
**Learning:** High STFT frame overlaps (e.g. hop_length=n_fft//4) are unnecessary for macroscopic heuristic aggregations. However, reducing overlap to 0% (hop_length=n_fft) is only safe for percentiles/means, as it causes blind spots for time-domain IIR filters and STFT bin-average RMS.
**Action:** Use hop_length=n_fft (0% overlap) for percentile/mean heuristic aggregations, and hop_length=n_fft//2 (50% overlap) for RMS/filter heuristics to safely gain 2x-4x speedups without losing accuracy.
