...
## 2026-07-05 - Replaced STFT with IIR Filter
**Learning:** Using `librosa.stft` just to calculate the RMS of high frequencies is computationally expensive because it recalculates the whole STFT for the file. A highpass IIR filter using `scipy.signal.sosfilt` is dramatically faster.
**Action:** Use fast IIR filtering (e.g., `scipy.signal.sosfilt`) for simple energy/RMS calculations instead of heavy STFT operations when precise phase or full spectrum data isn't needed.

## 2026-07-05 - Failed IIR optimization for STFT RMS
**Learning:** Replacing an STFT bin-average RMS calculation with a time-domain IIR filter changes the mathematical output entirely (average energy per bin vs total energy of the band), breaking downstream heuristics.
**Action:** When optimizing heuristic functions that aggregate STFT data, increase `hop_length` to reduce overlap instead of changing the fundamental math.
