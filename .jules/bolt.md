...
## 2026-07-20 - Optimize librosa stft
**Learning:** `librosa.stft` overlap overhead (e.g. `hop_length=512` when `n_fft=2048`) is unnecessary and extremely slow for coarse aggregate heuristics like calculating overall track RMS.
**Action:** Always set `hop_length=n_fft` when calculating non-temporal aggregate measurements over the entire signal using STFT.
