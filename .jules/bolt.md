...
## 2025-02-28 - Optimize macroscopic STFT RMS
**Learning:** Replacing an STFT bin-average RMS calculation with a time-domain IIR filter changes the mathematical output. To safely optimize macroscopic heuristic functions that aggregate STFT data (like high_band_rms_db), do not reduce overlap to 0% (`hop_length=n_fft`) as it causes blind spots with a Hann window or severe spectral leakage with a boxcar window. Instead, use a 50% overlap (`hop_length=n_fft//2`) for a safe 2x speedup without sacrificing signal integrity.
**Action:** Use `hop_length=n_fft//2` when calculating macroscopic RMS or signal aggregations with `librosa.stft`.
