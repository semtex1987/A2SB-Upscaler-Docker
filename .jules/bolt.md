...
## 2024-06-18 - Lazily load audio segments before resampling
**Learning:** When extracting short audio segments from large files, loading and resampling the entire file (e.g. 5 minutes) before slicing out a 2-second segment introduces massive memory and computation overhead.
**Action:** Compute segment boundaries with `librosa.get_duration`, then pass `offset` and `duration` to `librosa.load` to lazily load only the required frames. Request slightly more than needed (e.g., +0.05s) to guarantee enough frames survive resampling, then slice to exact length.
