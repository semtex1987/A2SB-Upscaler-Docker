...
## 2025-05-30 - Optimize librosa spectral features
**Learning:** `librosa.feature.spectral_flatness` and `librosa.feature.spectral_rolloff` are severe computational bottlenecks with default arguments (`hop_length=512`, `n_fft=2048`).
**Action:** Explicitly increase both `hop_length` and `n_fft` (e.g., to 2048) when calculating these features for macroscopic heuristic checks to remove the 75% STFT overlap, achieving a massive speedup (e.g., 4x to 2000x depending on the function).
