...
## 2024-05-24 - Lazy Loading Audio Segments
**Learning:** When extracting short audio segments from large files, reading the entire file into memory and processing it before cropping is a massive computational bottleneck, especially if resampling is involved.
**Action:** Use librosa.load with offset and duration arguments (which leverages soundfile's seek mechanism) to lazily fetch only the required frames. Compute valid boundaries first using librosa.get_duration to prevent out-of-bounds errors, and always explicitly pad or slice the output array to the exact expected length to handle slight deviations from floating-point rounding.
