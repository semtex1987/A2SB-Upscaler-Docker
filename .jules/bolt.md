...
## 2026-07-21 - Optimize high band energy calculation
**Learning:** Calculating a full STFT to measure the RMS energy of a specific frequency band is massively slower than using a time-domain IIR filter (like a Butterworth high-pass filter via `scipy.signal.sosfilt`). While adjusting STFT overlap can speed up STFTs, setting it to 0% with a Hann window causes severe attenuation at the window boundaries, destroying transient capture and causing functional regressions.
**Action:** Use cached `scipy.signal.butter` configurations and `scipy.signal.sosfilt` for simple time-domain energy measurements of frequency bands, avoiding STFT overhead completely while retaining functional accuracy.
