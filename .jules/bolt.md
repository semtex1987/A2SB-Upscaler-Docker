...
## 2026-07-26 - Optimize STFT Overlap for Macroscopic Heuristics
**Learning:** When aggregating STFT data over time (like in RMS calculations), reducing the default 75% frame overlap to 50% (`hop_length=n_fft//2`) provides a 2x speedup without sacrificing signal integrity or causing blind spots that occur with 0% overlap.
**Action:** Use 50% overlap for heuristic functions that aggregate STFT data instead of the default 75%.
