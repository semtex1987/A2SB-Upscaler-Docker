...
## 2025-02-28 - Lazy load audio segments to prevent O(N) loading overhead
**Learning:** Reading entire long audio files into memory and resampling them just to extract short clips introduces massive O(N) overhead in both compute (resampling) and memory usage per batch item. The original implementation was extracting small clips from huge files only after full load and resample.
**Action:** Use `librosa.get_duration()` to fast-calculate boundary validity, then utilize `librosa.load(offset=..., duration=...)` to lazily fetch only the needed segment frames, effectively turning O(N) data loading into O(1). Request a slightly larger duration to guarantee sufficient frames remain after resampling.
