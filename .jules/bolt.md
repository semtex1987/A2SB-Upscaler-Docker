...
## 2026-07-11 - Lazy load audio segments with librosa
**Learning:** When extracting short audio segments from large files, avoiding reading the entire file into memory with `librosa.load` defaults is critical. Passing `offset` and `duration` arguments utilizes `soundfile`'s seek mechanism under the hood, significantly speeding up data loading and reducing memory overhead. When resampling is also needed, fetch slightly more duration to ensure enough frames remain, then slice exactly.
**Action:** Always compute segment bounds and pass `offset` and `duration` to `librosa.load` when only a portion of the file is needed, avoiding O(N) memory allocation.
