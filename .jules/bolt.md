...
## 2025-02-28 - Fast librosa feature extraction
**Learning:** `librosa.feature.spectral_flatness` and `librosa.feature.spectral_rolloff` are severe computational bottlenecks with default arguments (`hop_length=512`, `n_fft=2048`). This triggers calculations with a 75% STFT overlap which is unnecessary for macroscopic heuristic checks.
**Action:** Explicitly increase both `hop_length` and `n_fft` (e.g., to 2048) when calculating these features to remove the overlap, achieving a massive speedup (e.g., 5x to 400x depending on the function).
