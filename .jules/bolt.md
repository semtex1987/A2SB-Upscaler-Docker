...
## 2024-05-18 - Optimize STFT overlap for macroscopic heuristic aggregations
**Learning:** When computing macroscopic heuristics that aggregate over time (e.g. percentiles or means of frequency bins like RMS), default 75% overlap (hop_length=n_fft//4) wastes computation. However, 0% overlap causes blind spots. A 50% overlap (hop_length=n_fft//2) offers a safe ~2x speedup without sacrificing test parity or accuracy.
**Action:** Use 50% overlap (`hop_length=n_fft//2`) for macroscopic aggregation functions to safely double speed while maintaining signal integrity.
