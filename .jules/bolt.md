...
## 2024-05-18 - Optimize spectral flatness computation
**Learning:** `librosa.feature.spectral_flatness` is a severe computational bottleneck with default arguments (`hop_length=512`, `n_fft=2048`). For macro-level metric approximations (like overall noise checks), high resolution is unnecessary.
**Action:** Always explicitly increase `hop_length` and `n_fft` (e.g., to 2048) when calculating spectral flatness if macro-level metrics are sufficient to drastically speed up computation.
