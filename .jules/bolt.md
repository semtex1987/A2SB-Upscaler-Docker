...
## 2026-06-14 - STFT Window Attenuation at 0% Overlap
**Learning:** When optimizing `librosa.stft` to run without frame overlap (e.g., `hop_length=n_fft`) to save compute during macroscopic heuristic calculations, the default Hann window (`window='hann'`) will erroneously attenuate the signal to zero at the frame boundaries. This leads to incorrect energy and magnitude aggregations since the edges aren't compensated by overlapping adjacent frames.
**Action:** When increasing `hop_length` to match `n_fft` to eliminate overlap overhead in STFT calculations, you must explicitly set `window='boxcar'` (a rectangular window) to ensure all signal energy is weighted equally and preserved.
