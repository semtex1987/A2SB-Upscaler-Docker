...
## 2024-05-18 - Lazy load audio segments
**Learning:** When extracting short audio segments from large files, using `librosa.load` without `offset` and `duration` reads the entire file into memory, which is a massive performance and memory bottleneck.
**Action:** Compute valid segment boundaries and lazily fetch only the required frames by passing `offset` and `duration` arguments to `librosa.load`. Always pad or slice the output array to the exact expected length to handle float rounding errors.
