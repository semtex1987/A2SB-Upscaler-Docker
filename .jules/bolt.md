...
## 2024-05-18 - Optimize librosa spectral feature extraction correctly
**Learning:** `librosa.feature.spectral_flatness`, `librosa.feature.spectral_rolloff`, and `librosa.stft` are computational bottlenecks with default arguments due to 75% overlap. Increase `hop_length` to equal `n_fft` (e.g., `hop_length=2048`, `n_fft=2048`) when calculating features for macroscopic heuristic checks that aggregate over time (like `np.percentile` or `mean(axis=1)`) to remove overlap overhead. Crucially, you must preserve the default `window='hann'` (or omit the `window` arg). Do NOT use `window='boxcar'`, as it introduces severe spectral leakage that pollutes high-frequency bins and ruins brickwall/boundary detection.
**Action:** When optimizing STFT calls for large-scale statistical analysis to reduce overlap, explicitly change `hop_length` but never change `window` from the default 'hann' without deep validation.
