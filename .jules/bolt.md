...
## 2025-02-28 - Avoid default overlap in librosa spectral features
**Learning:** `librosa.feature.spectral_flatness` and `librosa.feature.spectral_rolloff` are severe computational bottlenecks with default arguments (`hop_length=512`, `n_fft=2048`) because they compute STFTs with 75% overlap.
**Action:** When computing macroscopic heuristic checks using these functions, explicitly increase `hop_length` to match `n_fft` (e.g., to 2048) to remove overlap, making calculations roughly 4x faster without losing meaningful accuracy for these tasks.
