...
## 2026-07-25 - Optimize STFT Overlap for Macroscopic Heuristics
**Learning:** When applying STFT for macroscopic heuristics (like RMS calculation), using a default 75% overlap (hop_length=n_fft//4) creates unnecessary computational overhead. While reducing overlap speeds up execution, using 0% overlap (hop_length=n_fft) introduces severe blind spots with tapering window functions (like Hann). A 50% overlap (hop_length=n_fft//2) offers a safe 2x speedup over 75% without sacrificing signal integrity.
**Action:** Use a 50% overlap (hop_length=n_fft//2) for non-visual STFT-based macroscopic measurements.
