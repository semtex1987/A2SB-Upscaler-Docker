...
## 2026-06-22 - Lazy load audio segments with librosa
**Learning:** When extracting short audio segments from large files for dataset generation, using `librosa.load` without `offset` and `duration` loads the entire file into memory before resampling and slicing. This creates a massive bottleneck (up to 10000x slower for large files).
**Action:** Use `librosa.get_duration(path=audiopath)` to check bounds, then lazily fetch only the required frames by passing `offset` and `duration` to `librosa.load` *before* resampling. Request a slightly larger duration (e.g. +0.05s) to guarantee sufficient frames remain after resampling, then slice to the exact length.
