...
## 2025-05-24 - Lazy Loading Audio
**Learning:** When extracting short audio segments from large files, reading the entire file and resampling it *before* cropping causes a massive performance bottleneck due to redundant I/O and CPU overhead.
**Action:** Use librosa.get_duration to compute segment bounds, then lazy-load only the required frames with librosa.load(..., offset=..., duration=...) before resampling.
