...
## 2026-07-13 - Lazy Load Audio Segments
**Learning:** When extracting short audio segments from large files, using `librosa.load` defaults reads the entire file into memory and processes it, which is extremely inefficient. By computing valid segment boundaries with `librosa.get_duration` and passing `offset` and `duration` to `librosa.load`, we only process what is needed, resulting in a huge speedup.
**Action:** Always use lazy loading with `offset` and `duration` parameters in `librosa.load` when only a subsegment of an audio file is required.
