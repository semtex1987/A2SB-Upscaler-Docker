...
## 2024-10-25 - Optimize STFT overlap for heuristics
**Learning:** For macroscopic heuristic functions aggregating STFT data (like high-band RMS or HF edge detection), the default 75% overlap (`hop_length=n_fft//4`) is computationally wasteful. However, reducing overlap to 0% (`hop_length=n_fft`) causes blind spots with a Hann window.
**Action:** Use a 50% overlap (`hop_length=n_fft//2`) for a safe 2x speedup without sacrificing signal integrity. Never apply this optimization to STFTs used for visual features (e.g., spectrogram plots) as it drastically degrades time resolution.
