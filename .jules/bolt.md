...
## 2026-07-25 - Safely optimize STFT RMS aggregations
**Learning:** Replacing an STFT bin-average RMS calculation with a time-domain IIR filter changes the mathematical output entirely. To safely optimize macroscopic heuristic functions that aggregate STFT data (like high-band RMS), do not reduce overlap to 0% (`hop_length=n_fft`) as it causes blind spots with a Hann window. Instead, use a 50% overlap (`hop_length=n_fft//2`) for a safe 2x speedup without sacrificing signal integrity.
**Action:** For STFT-based RMS heuristics, use 50% overlap (`hop_length=n_fft//2`) instead of the default 75%, but avoid 0% overlap to maintain signal integrity.
