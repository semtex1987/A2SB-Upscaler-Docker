...
## 2025-02-28 - Lazy load audio segments
**Learning:** `librosa.load` on large audio files is very slow when the goal is to extract a small segment, particularly because resampling the entire array is O(N).
**Action:** Use `librosa.get_duration` to find valid boundary offsets, and then use `offset` and `duration` arguments to `librosa.load` to lazily fetch and resample only the required frames.
