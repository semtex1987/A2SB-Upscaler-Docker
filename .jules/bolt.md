...
## 2024-05-18 - Avoid Librosa STFT default overlaps for macroscopic features
**Learning:** `librosa.feature.spectral_flatness` and `librosa.feature.spectral_rolloff` are severe computational bottlenecks with default arguments (`hop_length=512`, `n_fft=2048`), which compute highly overlapping STFT frames. For macroscopic heuristics checking the entire output, this overlap is unnecessary.
**Action:** Explicitly increase both `hop_length` and `n_fft` (e.g., to 2048) when calculating these features to remove the 75% STFT overlap, achieving a massive speedup.
