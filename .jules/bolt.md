...
## 2026-07-20 - Safe STFT Overlap Reduction for Heuristics
**Learning:** To safely optimize macroscopic heuristic functions that aggregate STFT data (like high_band RMS), do not reduce overlap to 0% (hop_length=n_fft) as it causes blind spots with a Hann window. Instead, using a 50% overlap (hop_length=n_fft//2) provides a safe ~1.7x-2x speedup without sacrificing signal integrity.
**Action:** Use 50% overlap for STFTs used in macroscopic aggregations, but keep high overlap for visual features (like spectrograms).
