...
## 2026-07-01 - Lazy Loading Audio Segments with librosa
**Learning:** When extracting short audio segments from large files for dataset processing, using `librosa.load` defaults will eagerly read and resample the entire file into memory before slicing, which is extremely slow and memory intensive (O(N) operation).
**Action:** Always compute valid segment boundaries using `librosa.get_duration`, then lazily fetch only the required frames by passing `offset` and `duration` arguments to `librosa.load`. Request a slightly larger duration (e.g., duration_sec + 0.05) to ensure enough frames remain after resampling, and strictly slice to the exact length to prevent shape mismatches.
