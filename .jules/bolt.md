...
## 2025-02-28 - Optimize librosa STFT overlap
**Learning:** `librosa.stft` with default or small `hop_length` (e.g., `n_fft//4`) produces significant frame overlap overhead. For operations that aggregate data over time (like mean or RMS), this computes redundant frames and slows down processing considerably.
**Action:** Always set `hop_length=n_fft` and `window='boxcar'` when using `librosa.stft` for macroscopic heuristic checks that aggregate over time to eliminate overlap overhead.
