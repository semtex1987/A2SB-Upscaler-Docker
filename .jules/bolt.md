...
## 2025-02-28 - Lazy load audio segments safely
**Learning:** When extracting short audio segments from large files, avoiding reading the entire file into memory with `librosa.load` provides a massive speedup (e.g., 9000x for a 5 min file). However, due to floating-point rounding or soundfile internals during resampling, passing the exact `duration` can result in slightly fewer frames than expected.
**Action:** Compute valid segment boundaries using `librosa.get_duration`, then lazily fetch the frames with `librosa.load(..., offset=valid_start, duration=duration_sec + 0.05)` with a small buffer, and explicitly slice the output array to the exact required length (e.g., `audio = audio[:expected_length]`) to prevent downstream shape mismatch errors.
