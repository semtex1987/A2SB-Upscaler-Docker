...
## 2024-05-18 - Lazy load audio segments with librosa
**Learning:** When extracting short audio segments from large files, reading the entire file into memory with `librosa.load` without `offset` and `duration` arguments is a severe performance bottleneck. It forces unnecessary disk I/O and memory allocations for frames that will immediately be discarded.
**Action:** Always compute valid segment boundaries using `librosa.get_duration` to prevent out-of-bounds errors, then lazily fetch only the required frames by passing `offset` and `duration` arguments to `librosa.load` (which utilizes soundfile's seek mechanism under the hood).
