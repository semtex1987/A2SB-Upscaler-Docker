...
## 2026-07-08 - STFT Overlap for Heuristics
**Learning:** When calculating macroscopic heuristic functions that aggregate STFT data (like RMS), using a 50% overlap (hop_length=n_fft//2) instead of the default 75% overlap provides a ~2x speedup without sacrificing signal integrity or introducing blind spots.
**Action:** For macroscopic STFT aggregations, explicitly set hop_length=n_fft//2. Avoid doing this for visual features (spectrograms) as it degrades time resolution.
