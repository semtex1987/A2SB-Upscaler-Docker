...
## 2026-08-07 - Vectorizing Matrix Max-Pooling
**Learning:** When max-pooling large numpy matrices across an axis using predefined edge bins, using list comprehensions with `np.stack` and Python loops creates a massive bottleneck. Native vectorization with `np.maximum.reduceat` natively solves this and yields roughly a ~3x performance gain for spectrogram downsampling.
**Action:** Always prefer native `reduceat` operations over iterating block-slices when performing piece-wise aggregations across a dimension.
