...
## 2024-06-13 - Safe librosa STFT overlap for heuristics
**Learning:** When optimizing macroscopic heuristic functions that aggregate STFT data (like high-band RMS or spectral scan), do not reduce overlap to 0% (`hop_length=n_fft`) as it can slightly alter the calculated curve and fail strict test assertions.
**Action:** Use a 50% overlap (`hop_length=n_fft//2`) for a safe 2x speedup without sacrificing signal integrity or breaking parity tests.
