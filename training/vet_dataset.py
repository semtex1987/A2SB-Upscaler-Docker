...
def estimate_true_sr(y, sr, np, librosa) -> tuple[int, float]:
    ...
    # ⚡ Bolt: Increase hop_length/n_fft and use rectangular window to avoid default 75% overlap overhead
    rolloff_frames = librosa.feature.spectral_rolloff(
        y=seg, sr=sr, roll_percent=0.99, n_fft=2048, hop_length=2048, window="boxcar"
    )
