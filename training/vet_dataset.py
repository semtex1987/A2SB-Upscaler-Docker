...
def estimate_true_sr(y, sr, np, librosa) -> tuple[int, float]:
    ...
    # ⚡ Bolt: Use hop_length=n_fft to eliminate 75% overlap overhead while preserving default hann window accuracy
...
def hf_edge(y, sr, np, librosa) -> tuple[float, bool]:
    ...
    n_fft = 4096
    # ⚡ Bolt: Use hop_length=n_fft to eliminate 75% overlap overhead while preserving default hann window accuracy
    spec = np.abs(librosa.stft(y, n_fft=n_fft, hop_length=4096))
