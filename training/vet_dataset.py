...
def estimate_true_sr(y, sr, np, librosa) -> tuple[int, float]:
    ...
    # ⚡ Bolt: Set hop_length=n_fft (2048) to eliminate 75% overlap overhead for percentile check
...
def hf_edge(y, sr, np, librosa) -> tuple[float, bool]:
    ...
    n_fft = 4096
    # ⚡ Bolt: Increase hop_length to equal n_fft to remove overlap overhead for mean aggregation
