...
def estimate_true_sr(y, sr, np, librosa) -> tuple[int, float]:
    ...
    rolloff_frames = librosa.feature.spectral_rolloff(y=seg, sr=sr, roll_percent=0.99, n_fft=2048, hop_length=2048, window='boxcar')
...
def hf_edge(y, sr, np, librosa) -> tuple[float, bool]:
    ...
    n_fft = 4096
    spec = np.abs(librosa.stft(y, n_fft=n_fft, hop_length=n_fft, window='boxcar'))
