...
# ⚡ Bolt: Use soundfile.read directly for .wav files to avoid pydub's slow generic loading via ffmpeg
# ⚡ Bolt: Increase hop_length and n_fft in spectral_flatness to heavily reduce computation time for this macro-level check
def is_likely_corrupted_audio(path):
    try:
        samples, _ = soundfile.read(path, dtype="int16")
    except Exception:
        return True
