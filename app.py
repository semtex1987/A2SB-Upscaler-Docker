    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                gr.Markdown("Upload audio to restore, or provide staged paths below to bypass upload. Files are processed sequentially.", elem_classes=["text-sm", "text-gray-500", "mb-1"])
                input_files = gr.File(
                    file_count="multiple",
                    type="filepath",
                    file_types=["audio"],
                ...
                staged_paths = gr.Textbox(
                    ...
                    lines=2,
                    info="For pod use (e.g. runpodctl). Takes glob patterns.",
                )
                list_staged_button = gr.Button("List Staged Files")
                staged_preview = gr.Markdown("No staged files listed yet.")
                steps = gr.Slider(minimum=10, maximum=200, value=50, step=10, label="Steps (Quality)", info="50-100 recommended for bandwidth extension.")
                ...
                batch_size = gr.Slider(
                    ...
                    info="Lower batch size = less VRAM. 16-32 recommended for H100/H200.",
                )
