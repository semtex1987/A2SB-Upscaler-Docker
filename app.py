...
def is_likely_corrupted_audio(path):
    ...
    try:
        ...
        # librosa's spectral_flatness is slow with default args. We only need it
        # for a rough macro-level heuristic, so using larger n_fft/hop_length
        # speeds this up by ~4x without compromising the noise check accuracy.
