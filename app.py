        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                cutoff_choice = gr.Number(
                    ...
                    info="Set at or below where your source degrades. For MP3s, artifacts typically begin between 10–16 kHz depending on bitrate.",
                )
                batch_size = gr.Slider(
                    ...
                    info="Lower batch size reduces VRAM usage but takes longer. For H100/H200, 16-32 is a good starting range."
                )
                run_button = gr.Button("Process Batch", variant="primary")
                gr.Markdown("The results summary shows high-band energy before and after restoration to verify the model added content above the cutoff.", elem_classes=["text-sm", "text-gray-500", "mb-1"])
