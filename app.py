        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                gr.Markdown(
                    ...
                    "You may upload files here, or use the staged paths option below."
                )
                staged_paths = gr.Textbox(
                    label="Staged File Paths (Optional)",
                    placeholder="/app/inputs/*.wav\n/app/inputs/song_a.flac",
                    lines=2,
                    info="Optional: stage files directly on the pod (for example with runpodctl) to bypass browser upload."
                )
