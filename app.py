        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                staged_paths = gr.Textbox(
                    ...
                    lines=2,
                    info="Stage files directly on the pod (e.g. runpodctl) and enter paths here to bypass browser upload."
                )
                ...
                batch_size = gr.Slider(
                    ...
                    info="Lower reduces VRAM usage at the cost of longer inference time. For H100/H200, 16-32 is a good starting range."
                )
                run_button = gr.Button("Process Batch", variant="primary")
                gr.Markdown(
                    "The results summary shows high-band energy before and after restoration to verify the model added content above the cutoff.",
                    elem_classes=["text-sm", "text-gray-500", "mb-1"]
                )
