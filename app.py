    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                gr.Markdown(
                    "Upload one or many audio files to restore them sequentially. Supported formats: WAV, FLAC, MP3, OGG, M4A",
                ...
                staged_paths = gr.Textbox(
                    ...
                    lines=2,
                    info="Stage files directly on the pod (e.g. with runpodctl) and enter paths here to bypass browser upload.",
                )
                ...
                cutoff_choice = gr.Number(
                    ...
                    info="Set at or below where your source's content degrades. For MP3s, artifacts typically begin between 10-16 kHz depending on bitrate. Release checkpoints are weak above ~12 kHz.",
                )
                batch_size = gr.Slider(
                    ...
                    info="Lower reduces VRAM at the cost of time. For H100/H200, 16-32 is a good starting range.",
                )
                run_button = gr.Button("Process Batch", variant="primary")
                gr.Markdown(
                    "The results summary shows the high-band energy before and after restoration to verify added content above the cutoff.",
                    elem_classes=["text-sm", "text-gray-500", "mb-1"]
                )
