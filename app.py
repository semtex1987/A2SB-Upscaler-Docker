        ...
        with gr.Row():
            with gr.Column(scale=1):
                input_files = gr.File(
                    ...
                    info="Upload one or many audio files to restore them sequentially."
                )
                staged_paths = gr.Textbox(
                    label="Staged File Paths (Optional)",
                    placeholder="/app/inputs/*.wav\n/app/inputs/song_a.flac",
                    lines=2,
                    info="Optional: stage files directly on the pod (e.g. with runpodctl) and enter paths like /app/inputs/*.wav below to bypass browser upload."
                )
